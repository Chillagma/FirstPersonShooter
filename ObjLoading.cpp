...
public:
    ...
    pair<vector<GLdouble*> ,vector<GLdouble*>>function() {
        ...
        // Loop over shapes
        std::vector<GLdouble*> container, Ncontainer, container3;
            ...
            for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
                ...
                for (size_t v = 0; v < fv; v++) {
                    ...
                    if (idx.normal_index >= 0) {
                        ...
                        Ncontainer.push_back(new GLdouble[3]{ nx, ny, nz });
      ...
      //  foo(5);
        return std::make_pair(container, Ncontainer);
    };
};
...
public:
    void load(const char* filename) {
        ...
        for (std::string& line : lines) {
            if (line[0] == 'v') {
                if (line[1] == ' ') {
                    ...
                    normals.push_back(new GLdouble[3]{ a, b, c });
                    //std::cout<<normals.at(a) << std::endl;
                }
            }
            else if (line[0] == 'f') {
        ...
        aclass e;
     //   e.function();
        std::pair<vector<GLdouble*>, vector<GLdouble*>> container = e.function();

        Nclass n;
        //n.function();
        std::vector<GLdouble*> Ncontainer = n.function();

        std::vector<GLdouble*> transfer;
        ...
        for (Face& face : faces) {
            ...
            for (GLdouble* val2 : container.second)
            {
                transfer.push_back(val2);
              //glVertex3dv(val)
                /*
                                if (face.normal != -1)
            ...
            for (GLdouble* val : container.first)
            {
